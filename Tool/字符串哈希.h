class StringHash {
public:
  StringHash(int n = 1) : n(n), mult{{1, 1}} {
    assert(n > 0);
  ...
  std::vector<std::pair<u64, u64>> get_hash_set(const std::string &s) {
    u64 res1 = 0, res2 = 0;
  ...
  void add_hash_set(std::vector<std::pair<u64, u64>> &a, char c) {
    std::pair<u64, u64> tmp = {(a.back().first * m1 + c) % MOD[0],
                               (a.back().second * m2 + c) % MOD[1]};
    a.emplace_back(std::move(tmp));
  }

  bool equal(const std::vector<std::pair<u64, u64>> &a1, int l1, int r1,
             const std::vector<std::pair<u64, u64>> &a2, int l2, int r2) {
  ...
  std::pair<u64, u64> get_pair(const std::vector<std::pair<u64, u64>> &a1, int l1,
                               int r1) {
    // 很乱，我也不知道我在写什么
    return 
    {
      (
        a1[r1].first + MOD[0] - 
        (
          l1 == 0 ? 0 : a1[l1 - 1].first * pow(r1 + 1 - l1, MOD[0]) % MOD[0]
        )
      ) % MOD[0]
      ,
      (
        a1[r1].second + MOD[1] - 
        (
          l1 == 0 ? 0 : a1[l1 - 1].second * pow(r1 + 1 - l1, MOD[1]) % MOD[1]
        )
      ) % MOD[1]
    };
  }
...
private:
  ...
  int n;
  u64 m1, m2;
  std::vector<std::pair<u64, u64>> mult;

  void init_mult(int m) {
    mult.resize(m + 1);
    for (int i = n; i <= m; i++) {
